...
// import { deleteEmployee } from "../Databse/Db";
...
const Employee = (props) => {
  const [elpData, setElpData, name, setName,email, setEmail, no, setNo, des, setDes, dep, setDep,gen, setGen] = useContext(Context)
  ...
  return (
    <div className="employee detail-info"  id={props.id} >
      <figure>
        <img
          src={props.gen === "male" ? maleURL : femaleURL}
          alt="profile"
        ></img>
      </figure>
      <h4>{props.name}</h4>
      ...
      <div className="btn">
          ...
          <button type="submit" className="view" id={props.id} onClick={emp}>VIEW</button>
          <button type="submit" className="edit" id={props.id} onClick={emp}>Edit</button>
        
        <button type="submit" id={props.id}   onClick={event => props.child(event)}>
          Delete
        </button>
      </div>
    </div>
